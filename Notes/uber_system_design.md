...
When rider reconnects, they might land on a DIFFERENT gateway server.
    ...
    │   Rider receiving location updates ✓                                    │
    │                                                                         │
...
───────────────────────────────────────────────────────────────────────────────
    ...
    │  ────────────────────────────────────                                  │
    │  ✓ High contention (many requests for same resource)                  │
    │  ✓ Retry is expensive (e.g., re-running matching algorithm)           │
    │  ✓ Critical section is short (don't hold lock long)                   │
    │  ✓ You need to guarantee only one proceeds                            │
    │                                                                         │
    │  Examples in Uber:                                                     │
    ...
    │  ✓ Retry is cheap (just read and try again)                          │
    │  ✓ Read-heavy workload (many reads, few writes)                       │
    │  ✓ Long-running operations (don't want to block others)               │
    │                                                                         │
    │  Examples in Uber:                                                     │
    ...
    │  ────────────────────────────────                                      │
    │  ✓ Data integrity is CRITICAL (payments, driver assignment)           │
    │  ✓ Distributed lock might fail (network issues)                       │
    │  ✓ You need guarantees at multiple levels                             │
    │                                                                         │
    │  Pattern:                                                              │
...
A client can BELIEVE it holds the lock when it actually doesn't.
    ...
    │     │                       │                        │                  │
    │     │                       │                        │                  │
    │     │── Write (token 43) ────────────────────────────►│ ✓ Accepted     │
    │     │                       │                        │                  │
    │     │   [A resumes]         │                        │                  │
    │     │                       │                        │                  │
    │     │── Write (token 42) ────────────────────────────►│ ✓ Accepted!    │
    │     │                       │                        │  CORRUPTION!    │
    │                                                                         │
...
───────────────────────────────────────────────────────────────────────────────
    ...
    │                                                                         │
    │  T1: Mark ride COMPLETED    ✓ Commit to Ride DB                        │
    │  T2: Calculate fare         ✓ Read-only, no commit needed              │
    │  T3: Charge rider $20       ✓ Commit to Payment DB                     │
    │  T4: Credit driver $16      ✓ Commit to Earnings DB                    │
    │  T5: Send receipt           ✓ Email sent                               │
    │                                                                         │
    ...
    │                                                                         │
    │  T1: Mark ride COMPLETED    ✓ Committed                                │
    │  T2: Calculate fare         ✓ Done                                     │
...
───────────────────────────────────────────────────────────────────────────────
    ...
    │                                                                         │
    │  ORCHESTRATOR:                                                         │
    │  1. Calls Ride Service: "Mark completed"          ✓                    │
...
───────────────────────────────────────────────────────────────────────────────
    ...
    │                                                                         │
    │  WHY IT WORKS:                                                         │
    │  • Same ride_id + attempt = same key on retry ✓                       │
...
───────────────────────────────────────────────────────────────────────────────
    ...
    │  2. Called payment gateway, card charged ✓                            │
