...
|                                                                         |
|  CANDIDATE: Are we designing a full URL shortener like bit.ly, or       |
|    just the redirect service?                                           |
|                                                                         |
|  INTERVIEWER: The full system. Create short URLs, redirect users,       |
|    and track click analytics.                                           |
|                                                                         |
|  -----------------------------------------------------------------      |
|                                                                         |
|  CANDIDATE: What scale? How many URLs created and redirected per day?   |
|                                                                         |
|  INTERVIEWER: 100M new URLs/day, 10B redirects/day. Read-heavy          |
|    (100:1 read-to-write ratio).                                         |
|                                                                         |
|  -----------------------------------------------------------------      |
|                                                                         |
|  CANDIDATE: Should short URLs expire, or live forever?                  |
|                                                                         |
|  INTERVIEWER: Configurable TTL. Default: never expire. Support          |
|    custom expiration for temporary links.                               |
|                                                                         |
|  -----------------------------------------------------------------      |
|                                                                         |
|  AGREED SCOPE:                                                          |
|                                                                         |
|  * URL shortener: create, redirect, analytics                           |
|  * 100M creates/day, 10B redirects/day                                  |
|  * Base62 encoding for short codes                                      |
|  * Configurable TTL for link expiration                                 |
|  * Deep dive: ID generation + caching for redirect performance          |
|                                                                         |
+-------------------------------------------------------------------------+
```
...
|                                                                         |
|  1. BASE62 ENCODING of a unique ID to generate short codes. 7-char      |
|     code supports 3.5 trillion unique URLs.                             |
|  2. UNIQUE ID GENERATOR: auto-increment (single DB) or distributed      |
|     (Snowflake). Snowflake for horizontal scaling.                      |
|  3. REDIS CACHE for redirect (cache the mapping short -> long).         |
|     99%+ cache hit rate since popular URLs are accessed repeatedly.     |
|  4. 301 vs 302 REDIRECT: 301 (permanent) is cached by browser           |
|     (fewer server hits but no analytics). 302 (temporary) always        |
|     hits server (better analytics tracking). We use 302.                |
|                                                                         |
|  -----------------------------------------------------------------      |
|                                                                         |
|  KEY TRADE-OFFS:                                                        |
|                                                                         |
|  * HASH vs COUNTER for ID: Hash (MD5/SHA) may collide, needs dedup      |
|    check. Counter is unique but sequential (predictable URLs).          |
|    Base62-encoded counter with random offset is the best balance.       |
|  * 301 vs 302 REDIRECT: 301 saves server load (browser caches) but      |
|    loses analytics data. 302 always tracks but more server hits.        |
|    Analytics is core to the business, so 302 wins.
|                                                                         |
+-------------------------------------------------------------------------+
```
